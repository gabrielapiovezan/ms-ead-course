...
public interface LessonRepository extends JpaRepository<LessonModel, UUID> {
    ...
    List<LessonModel> findAllLessonsIntoModule(@Param("moduleId") String moduleId);

    @Query(value = "SELECT * FROM tb_lessons WHERE module_module_id = :moduleId AND lesson_id = :lessonId", nativeQuery = true)
    Optional<LessonModel> findLessonIntoModule(@Param("moduleId") String moduleId, @Param("lessonId") String lessonId);
}
